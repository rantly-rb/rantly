...
void init(const char *s)
{
  ...
  fprintf(fp, "%8d\n", 0);      // low water
  fprintf(fp, "%8d\n", 0);      // high water
  for (i = 0; i < size; i++)    // BUG: we need one extra entry
    fprintf(fp, "%8d\n", -1);   // data
  fclose(fp);
}
...
void put(const char *s)
{
  ...
  high++; if (high >= size) high = 0;   // BUG: we need one extra entry
  if (high == low)
  {
...
void get()
{
  ...
  fscanf(fp, "%d\n", &value);
  low++; if (low >= size) low = 0;       // BUG: we need one extra entry
...
void length()
{
  ...
  if (low <= high)
    ...
    printf("%d\n", size + high - low);      // BUG: we need one extra entry
  fclose(fp);
}
